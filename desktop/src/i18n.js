...
(function createMiniClipI18n() {
  ...
  const en = {
    ...
    '＋ 导入媒体': '+ Import Media', '▦ 素材库': '▦ Media', '插入': 'Insert', '覆盖': 'Overwrite',
    ...
    '在下方时间线点选一个片段进行编辑': 'Select a clip in the timeline below to edit it', '片段名称': 'Clip Name', '为片段命名…': 'Name this clip…',
    '项目素材': 'Project Media', '0 个素材': '0 media items', '＋ 导入到素材库': '+ Import to Library', '搜索素材…': 'Search media…', '搜索素材': 'Search media', '筛选素材类型': 'Filter media type', '素材排序': 'Sort media', '全部': 'All', '导入顺序': 'Import Order', '名称': 'Name', '类型': 'Type', '追加主轨': 'Append to Main', '插入播放头': 'Insert at Playhead', '覆盖片段': 'Overwrite Clip', '作为叠加': 'Add as Overlay', '作为视频层': 'Add as Video Layer', '加入音频轨': 'Add to Audio Track', '设为背景音乐': 'Set as Background Music', '移出素材库': 'Remove from Library', '导入视频、图片或音频后，可反复拖放或添加到时间线。': 'Import video, images, or audio, then drag or add them to the timeline.', '没有匹配的素材。': 'No matching media.', '也可拖到主视频、视频层、叠加或音频轨。': 'You can also drag media to the main video, video layer, overlay, or audio track.', '隐藏素材库': 'Hide media library', '显示或隐藏项目素材库': 'Show or hide project media library',
    ...
    '时间线': 'Timeline', '标记': 'Marker', '🧲 磁吸': '🧲 Snap', '○ 磁吸': '○ Snap', '◆ 标记': '◆ Marker', '删除标记': 'Delete Marker', '标记注释…': 'Marker note…', '标记注释': 'Marker note', '跳转时间…': 'Go to time…', '跳转时间': 'Go to time', '输入秒数或时间码后回车跳转': 'Enter seconds or a timecode, then press Enter to jump', '适配': 'Fit', '还没有片段，点右上角「导入视频」': 'No clips yet. Click “Import Media” in the top-right corner.',
    ...
    '缩小时间线': 'Zoom out timeline', '放大时间线': 'Zoom in timeline', '缩小时间线（Ctrl/⌘ + 滚轮也可缩放）': 'Zoom out timeline (Ctrl/⌘ + wheel also zooms)', '放大时间线（Ctrl/⌘ + 滚轮也可缩放）': 'Zoom in timeline (Ctrl/⌘ + wheel also zooms)', '关闭时间线磁吸': 'Disable timeline snapping', '开启时间线磁吸': 'Enable timeline snapping', '适配整条时间线（Shift+Z）': 'Fit entire timeline (Shift+Z)', '拖动裁剪片段起点': 'Drag to trim clip start', '拖动裁剪片段终点': 'Drag to trim clip end', '在播放头添加标记（M）': 'Add marker at playhead (M)', '跳转到上一个标记': 'Jump to previous marker', '跳转到下一个标记': 'Jump to next marker', '删除选中的标记': 'Delete selected marker', '时间线标记': 'Timeline markers', '锁定主视频轨': 'Lock main video track', '解锁主视频轨': 'Unlock main video track',
  ...
  const dynamic = [
    ...
    [/^播放头位置已有标记$/, 'There is already a marker at the playhead'],
    [/^请先选择一个标记$/, 'Select a marker first'],
    [/^已删除标记$/, 'Marker deleted'],
    [/^时间码格式无效$/, 'Invalid timecode'],
    [/^播放头不在可裁剪片段内$/, 'The playhead is not inside a trimmable clip'],
    [/^播放位置太靠近片段边缘，无法设置入点$/, 'The playhead is too close to the edge to set an in point'],
    [/^播放位置太靠近片段边缘，无法设置出点$/, 'The playhead is too close to the edge to set an out point'],
    [/^已将片段入点设为播放头位置$/, 'Set the clip in point at the playhead'],
    [/^已将片段出点设为播放头位置$/, 'Set the clip out point at the playhead'],
    [/^已裁剪片段$/, 'Clip trimmed'],
    [/^播放头之前没有标记$/, 'There is no marker before the playhead'],
    [/^播放头之后没有标记$/, 'There is no marker after the playhead'],
    [/^已在 (.*)s 添加标记$/, 'Added a marker at $1 s'],
    ...
    [/^已复制选中主视频片段$/, 'Copied the selected main video clip'],
    [/^(\d+) 个素材$/, '$1 media item(s)'],
    [/^(\d+) 个素材 · 显示 (\d+) 个$/, '$1 media item(s) · showing $2'],
    [/^素材库新增 (\d+) 个素材$/, 'Added $1 new item(s) to the media library'],
    [/^没有可加入素材库的媒体$/, 'No usable media was selected for the media library'],
    [/^已从素材库追加：(.*)$/, 'Appended from media library: $1'],
    [/^已从素材库插入 (\d+) 段视频，并波纹后移后续内容$/, 'Inserted $1 clip(s) from the media library and rippled later content'],
    [/^已用素材库素材覆盖：(.*)$/, 'Overwrote with media library item: $1'],
    [/^已从素材库添加叠加：(.*)$/, 'Added overlay from media library: $1'],
    [/^已从素材库添加视频层：(.*)$/, 'Added video layer from media library: $1'],
    [/^已从素材库添加音频：(.*)$/, 'Added audio from media library: $1'],
    [/^已将素材设为背景音乐：(.*)$/, 'Set media as background music: $1'],
    [/^已从素材库移除：(.*)$/, 'Removed from media library: $1'],
    [/^素材正在时间线中使用，不能移出素材库$/, 'This media is in use on the timeline and cannot be removed'],
    [/^已在播放头粘贴 (\d+) 段视频，并波纹后移后续内容$/, 'Pasted $1 video clip(s) at the playhead and rippled later content'],
